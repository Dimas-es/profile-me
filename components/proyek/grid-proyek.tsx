...
import { getPersonalProjects, getTeamProjects } from "@/utils/data-provider"
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <h2 className="text-xl sm:text-2xl font-bold text-gray-900">{"Proyek Sophia"}</h2>
      ...
      <div className="space-y-4 sm:space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <h2 className="text-xl sm:text-2xl font-bold text-gray-900">Proyek Tim</h2>
          <div className="relative w-full sm:w-auto">
            <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 w-4 h-4" />
            <Input placeholder="Cari" className="pl-10 w-full sm:w-64" />
          </div>
          ...
          {teamProjects.map((project) => (
            <Card key={project.id} className="overflow-hidden hover:shadow-lg transition-shadow">
              ...
              <CardContent className="p-4 sm:p-5">
                ...
                <p className="text-blue-600 text-xs sm:text-sm font-medium mt-1">Peran: {project.role}</p>
